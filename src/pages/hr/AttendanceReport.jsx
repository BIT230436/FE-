...
import { DatePicker, Table, Button, Select, Input, Card, Row, Col, Statistic, message, Spin } from 'antd';
import moment from 'moment';
import { SearchOutlined, FileExcelOutlined, FilterOutlined, LoadingOutlined } from '@ant-design/icons';
...
const AttendanceReport = () => {
  ...
  const generateMockData = () => {
    const names = [
      ...
      'Hoàng Văn Đạt', 'Vũ Thị Hà', 'Đặng Văn Hùng', 'Bùi Thị Hương',
      'Đỗ Văn Khánh', 'Nguyễn Thị Lan', 'Trần Văn Minh', 'Lê Thị Ngọc'
    ];
  ...
  const fetchAttendanceData = async () => {
    try {
      ...
      const summaryData = attendanceData.reduce(
        (acc, curr) => ({
          totalWorkingDays: acc.totalWorkingDays + (parseInt(curr.workingDays) || 0),
          totalLeaveDays: acc.totalLeaveDays + (parseInt(curr.leaveDays) || 0),
          totalLateDays: acc.totalLateDays + (parseInt(curr.lateDays) || 0),
          totalAbsentDays: acc.totalAbsentDays + (parseInt(curr.absentDays) || 0),
        }),
        { totalWorkingDays: 0, totalLeaveDays: 0, totalLateDays: 0, totalAbsentDays: 0 }
      );
      ...
      message.warning('Đang sử dụng dữ liệu mẫu. Vui lòng kiểm tra kết nối API.');
    } finally {
      setLoading(false);
    }
  };
